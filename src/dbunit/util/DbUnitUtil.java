...
public class DbUnitUtil {
  ...
  public static void setUpExistSchema(final String initDbXmlFileName)
      throws MalformedURLException, DataSetException, ClassNotFoundException, Exception {
    databaseTester =
        new JdbcDatabaseTester(DRIVER_CLASS, CONNECTION_URL, USER_NAME, PASSWORD, SCHEMA);
    setUpCommon(initDbXmlFileName);
  }
   ...
   *
   * @param expectedDbXmlFile 想定する結果を記載したXMLファイル名
   ...
   * @throws Exception Exception
   */
  public static void check(final String expectedDbXmlFile, final String tableName,
      final String[] dateColumnArray)
    ...
    ITable filteredExpectedTable =
        DefaultColumnFilter.excludedColumnsTable(expectedTable, dateColumnArray);
    ITable filteredActualTable =
        DefaultColumnFilter.excludedColumnsTable(actualTable, dateColumnArray);
